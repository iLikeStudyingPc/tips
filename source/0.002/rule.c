...
int whitch_command(char *str, struct command_opention *thisopention) {
  ...
  if(strcmp(str,"-new")==0){/*恢复*/
    wprintf(L"find -new\n");
    thisopention->new=1;
    return 1;
  }
  if(strcmp(str,"-old")==0){/*恢复*/
    wprintf(L"find -old\n");
    thisopention->old=1;
    return 1;
  }
  if(strcmp(str,"-backup")==0){/*恢复*/
...
//(%ls)The text editor software was not found. Please set a new text editor software name:
/*
